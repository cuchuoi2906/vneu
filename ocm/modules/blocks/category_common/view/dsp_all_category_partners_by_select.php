...
<form action="<?php html_link('ajax/'.$this->className().'/act_update_category_khampha_by_news'); ?>" method="post" name="frm_dsp_all_item" style="padding-top:10px" target="frm_submit">
	...
	<a class="button_big btn_grey" href="javascript:;" onclick="return frm_submit_select_category(document.frm_dsp_all_item, 'sel_category_<?php echo $v_code; ?>_list');">Chọn</a>
    ...
    <div style="width:100%;height:325px;margin-bottom:10px;overflow-y:auto">
        <table width="100%" cellpadding="1" cellspacing="1" border="1" bordercolor="#bbb" style="border-collapse:collapse">
            ...
            for ($i=0; $i<$v_record_count; $i++) {
                ...
                $v_isCheck = (in_array($v_category['fk_category_id'], $v_arr_category)) ? 'checked' : '';
                ?>
                <tr>
                    <td class="<?php echo $v_class_name1; ?>" align="center">
                        <input type="checkbox" name="chk_item_id_<?php echo $i; ?>" id="item_id_<?php echo $v_category['fk_category_id']; ?>" value="<?php echo $v_category['fk_category_id']; ?>" <?php echo $v_isCheck; ?> />
                        <input type="hidden" name="hdn_category_name_<?php echo $i; ?>" value="<?php echo  htmlspecialchars(str_replace('&nbsp;&nbsp;&nbsp;&nbsp;', '-- ',$v_category['c_name'])); ?>" />
                    </td>
                    <td class="<?php echo $v_class_name2; ?>">
                        <label for="item_id_<?php echo $v_category['fk_category_id']; ?>"><?php echo str_replace('&nbsp;&nbsp;&nbsp;&nbsp;', '', $v_category['c_name']); ?></label>
                    </td>
                </tr>
                <?php
            }
