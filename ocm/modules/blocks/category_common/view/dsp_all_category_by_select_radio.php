...
<form action="#" method="post" name="frm_dsp_all_item" style="padding-top:10px" target="frm_submit">
    <input type="hidden" id="hdn_record_count" name="hdn_record_count" value="<?php echo $v_record_count; ?>" />
	<a class="button_big btn_grey" href="javascript:;" onclick="return radio_select_category_and_close_window(document.frm_dsp_all_item, '<?php echo $v_target_id; ?>', '<?php echo $p_div_to_put_source_from; ?>','<?php echo $p_div_to_insert_ten_box; ?>','<?php echo $p_class_check_da_duoc_chon; ?>')">Chọn</a>
    <a class="button_big btn_grey" href="javascript:;" onclick="window.close()">Đóng</a>
	<span>
		<label>
			<input type="radio" name="rad_option"  id = "rad_option1" <?php echo ($v_option!=1)? 'checked':'';?> onclick="window.location.href='<?php html_link($this->className().'/dsp_all_category_by_select_radio/?'.htmlentities($v_url_reload_default)); ?>';" />
			Tất cả
		</label>
		<label>
			<input type="radio" name="rad_option"  id = "rad_option2" <?php echo ($v_option!=1)? '':'checked';?> onclick="window.location.href='<?php html_link($this->className().'/dsp_all_category_by_select_radio/?'.htmlentities($v_url_reload)); ?>';" />
    ...
    <div style="width:100%;height:325px;margin-bottom:10px;overflow-y:auto">
        <table width="100%" cellpadding="1" cellspacing="0" border="1" bordercolor="#bbb" style="border-collapse:collapse">
            <tr>
                <td class="popup-chuyenmuc" width="20" align="center"></td>
            ...
            for ($i=0; $i<$v_record_count; $i++) {
                ...
                <tr>
                    <td class="<?php echo $v_class_name1; ?>" align="center">
                        <input type="radio" name="chk_item_id_<?php echo $v_target_id; ?>" id="item_id_<?php echo $v_category['ID']; ?>" value="<?php echo $v_category['ID']; ?>" <?php echo $v_isCheck; ?> />
                        <input type="hidden" name="hdn_category_name_<?php echo $v_category['ID']; ?>" value="<?php echo  htmlspecialchars(str_replace('&nbsp;&nbsp;&nbsp;&nbsp;', '-- ',$v_category['Name'])); ?>" />
                    </td>
