...
function onMagazineTemplateChange(e, template_id) {
  ...
  // kiểm tra ko cho chọn template khác khi đã chọn template slide
  var v_magazine_id = parseInt(document.getElementById('magazine_id').value);
  if(parseInt(v_is_temp_slide) == 1 && parseInt(v_id_temp_slide_current) != template_id && v_magazine_id > 0){
      alert('Content đã chọn template slide story. Không được chọn template khác!');
      return false;
  }
  if (old_value > 0) {
  ...
  $.ajax({
    ...
    success: function(html) {
      ...
      if(loadzone.length) {
        ...
        setTimeout(function() {
            ocm_chay_javascript_tu_ket_qua_ajax(html);
            if(jscolor !== '' && jscolor !== undefined){
                if(jscolor.binding) {
                    jscolor.bind();
                }
...
function addNewContent(e) {
  ...
  var base_url = that.attr('data-base-url');
  var v_is_temp_slide_add = v_is_temp_slide;
  var v_id_temp_slide_current_add = v_id_temp_slide_current;
  $.ajax({
    type: 'GET',
    ...
    success: function(html) {
      ...
      if (loadzone.length) {
        ...
        if(parseInt(v_is_temp_slide_add) > 0 && parseInt(v_id_temp_slide_current_add) > 0){
            v_is_temp_slide = v_is_temp_slide_add;
            v_id_temp_slide_current = v_id_temp_slide_current_add;
            ocm_chay_javascript_tu_ket_qua_ajax(html);
        }
      }
    }
...
function checkFile(elem) {
  ...
  if(file) {
    ...
    if (type == 'image') {
      if (metadata.width) {
        if (!checkByOperator(width, metadata.width.operator, metadata.width.value)) {
          var msg = 'Chiều rộng ảnh' +' \'' + file.name + '\' không hợp lệ. Chiều rộng ảnh phải ' + getTextByOperator(metadata.width.operator, metadata.width.value);
      ...
      if (metadata.height) {
        if (!checkByOperator(height, metadata.height.operator, metadata.height.value)) {
          var msg = 'Chiều cao ảnh' +' \'' + file.name + '\' không hợp lệ. Chiều cao ảnh phải ' + getTextByOperator(metadata.height.operator, metadata.height.value);
          that.parent().parent().find('.error-map-element').html(msg);
          return false;
        }
      }
        // check tỷ lệ ảnh
        if (metadata.poster_video) {
            var arr_value_poster = metadata.poster_video.value.split(",");
            var ratio_poster = arr_value_poster[0]/arr_value_poster[1];
            var ratio_poster_file = width/height;
            if(ratio_poster != ratio_poster_file){
                var msg = 'Tỷ lệ ảnh đại diện video: ' +' \'' + file.name + '\' không hợp lệ. Tỷ lệ ảnh đại diện video phải: '+arr_value_poster[0] +':'+ arr_value_poster[1];
                that.parent().parent().find('.error-map-element').html(msg);
                return false;
            }
        }
    }
    // validate riêng đối với video
    if(type == 'video'){
        if (metadata.width) {
            if (parseInt(width) > parseInt(metadata.width.value)) {
                var msg = 'Chiều rộng video' +' \'' + file.name + '\' không hợp lệ. Chiều rộng video phải ' + getTextByOperator(metadata.width.operator, metadata.width.value);
                that.parent().parent().find('.error-map-element').html(msg);
                return false;
            }
        }
        if (metadata.height) {
            if (parseInt(height) > parseInt(metadata.height.value)) {
                var msg = 'Chiều cao video' +' \'' + file.name + '\' không hợp lệ. Chiều cao video phải ' + getTextByOperator(metadata.height.operator, metadata.height.value);
                that.parent().parent().find('.error-map-element').html(msg);
                return false;
            }
        }
    ...
    // validate file size
    if (metadata.file_size && !metadata.file_size_gif) {
    ...
    // check đối với ảnh gif
    if (metadata.file_size_gif && !metadata.file_size) {
        if (!checkByOperator(file_size_gif, metadata.file_size_gif.operator, human2byte(metadata.file_size_gif.value))) {
            var msg = 'File \'' + file.name + '\' vượt quá dung lượng cho phép. Dung lượng tối đa ' + metadata.file_size_gif.value ;
            that.parent().parent().find('.error-map-element').html(msg);
            return false;
        }
    }
    ...
    // chỉ báo chưa chọn file nếu không phải là ảnh poster của video
    if (metadata.poster_video === undefined && metadata.type_background === undefined) {
...
function validateFileExtension(that, file) {
  ...
  //Begin AnhTT 22/4/2020 toi_uu_magazine
    //Thêm đuôi JPG
  if(file.type == 'JPG' || file.type == 'image/jpeg' || file.type == 'image/gif'){
      return true;
  }
  //Begin AnhTT 22/4/2020 toi_uu_magazine
...
// BEGIN 23/7/2019 tuannt so sng chuc nang di chuyen len xuong khoi noi dung magazine
    ...
    function move(element,pos,check) {
        // Kiểm tra thao tác chưa hoàn thành thì thông báo
        if (v_allow_action_down_up == 2) {
            alert('Bạn hãy đợi 1 giây để dữ liệu thay đổi');
            return;
        }
        // Gắn cờ để không cho phép thao tác cho đến khi hoàn thành chuyển đổi
        if (v_allow_action_down_up == 1){
            v_allow_action_down_up = 2;
        }
        var move = 0;
        // nếu là chuyển xuống
        if(check == 0){
            ...
            var posid = pos + 1;
            var pos_v = posid;
            var getid_value = posval - 1;
            // kiểm tra xem có phần tử nào phía dưới không
        ...
        // nếu là chuyển lên
            ...
            var pos_v = pos +1;
            var getid_value = posval + 1;
             // kiểm tra xem có phần tử nào phía trên không
            if(element.previousElementSibling){    
               move = 1;
            }
        }
        // nếu có phần tử phía dưới hoặc trên (tuỳ vào trường hợp chuyển lên hay xuống)
        if(move != 0){
            // get html iframe preview
            var ifr = document.getElementById('preview__'+pos).contentWindow.document.body.innerHTML;
            var ifr1 = document.getElementById('preview__'+posid).contentWindow.document.body.innerHTML;
             var loadzl = new Array();
            var loadzl1 = new Array();
            for(i = 0;i< 100;i++){    
                // get html ckeditor
                // nếu cả 2 phần tử cạnh nhau đều k có text box ckeditor thì thoát khỏi vòng lặp
              if(($("#map__paragraph_"+i+"__"+pos).html() == null || $("#map__paragraph_"+i+"__"+pos).html() == 'null' || $("#map__paragraph_"+i+"__"+pos).html() == 'undefined') && ($("#map__paragraph_"+i+"__"+posid).html() == null || $("#map__paragraph_"+i+"__"+posid).html() == 'null' || $("#map__paragraph_"+i+"__"+posid).html() == 'undefined')){
                  break;
              }
              loadzl[i] = $("#map__paragraph_"+i+"__"+pos).html();
              loadzl1[i] = $("#map__paragraph_"+i+"__"+posid).html();
            }
            if(move == 2){                
            ...
            // Thay đổi id và giá trị các div liên quan
          $('#getid'+pos).html(posval);
          $('#getid'+posid).html(getid_value);
          $('#up-'+pos).attr('onclick', "no2up('MagazineContent"+posid+"',"+posid+")");
          $('#down-'+pos).attr('onclick', "no2down('MagazineContent"+posid+"',"+posid+")");
											  
          $('#up-'+posid).attr('onclick', "no2up('MagazineContent"+pos+"',"+pos+")");
          $('#down-'+posid).attr('onclick', "no2down('MagazineContent"+pos+"',"+pos+")");          
          $('#MagazineContent'+pos).attr('id', 'MagazineContent');
          $('#MagazineContent'+posid).attr('id', 'MagazineContent'+pos);
          $('#MagazineContent').attr('id', 'MagazineContent'+posid);
          $('#preview__'+pos).attr('id', 'preview__');
          $('#preview__'+posid).attr('id', 'preview__'+pos);
          $('#preview__').attr('id', 'preview__'+posid);
          $('#LoadZoneLeft_'+pos).attr('id', 'LoadZoneLeft_');
          $('#LoadZoneLeft_'+posid).attr('id', 'LoadZoneLeft_'+pos);
          $('#LoadZoneLeft_').attr('id', 'LoadZoneLeft_'+posid);
											 
												  
											   
												   
          $('#up-'+pos).attr('id', 'up-');
          $('#up-'+posid).attr('id', 'up-'+pos);
          $('#up-').attr('id', 'up-'+posid);
          $('#down-'+pos).attr('id', 'down-');
          $('#down-'+posid).attr('id', 'down-'+pos);
          $('#down-').attr('id', 'down-'+posid);
          $('#getid'+pos).attr('id','getid');
          $('#getid'+posid).attr('id','getid'+pos);
          $('#getid').attr('id','getid'+posid);
          $('#position__'+pos).val(posval);
          $('#position__'+pos).attr('value', posval);
          $('#position__'+posid).val(pos_v);
          $('#position__'+posid).attr('value', pos_v);
          $('#position__'+pos).attr('id', 'position__');
          $('#position__'+posid).attr('id', 'position__'+pos);
          $('#position__').attr('id', 'position__'+posid);          
          for(i=0;i<100;i++){
              if(document.getElementById("map__paragraph_"+i+"__"+pos) && document.getElementById("map__paragraph_"+i+"__"+posid)){
                $("#map__paragraph_"+i+"__"+pos).attr('name', "map__paragraph_"+i+"__");
                $("#map__paragraph_"+i+"__"+pos).attr('id', "map__paragraph_"+i+"__");
                $("#map__paragraph_"+i+"__"+posid).attr('name', "map__paragraph_"+i+"__"+pos);  
                $("#map__paragraph_"+i+"__"+posid).attr('id', "map__paragraph_"+i+"__"+pos);
																											  
                $("#map__paragraph_"+i+"__").attr('name', "map__paragraph_"+i+"__"+posid);
                $("#map__paragraph_"+i+"__").attr('id', "map__paragraph_"+i+"__"+posid);
                
              }
              else if (document.getElementById("map__paragraph_"+i+"__"+pos) && !document.getElementById("map__paragraph_"+i+"__"+posid)){
                  $("#map__paragraph_"+i+"__"+pos).attr('name', "map__paragraph_"+i+"__"+posid);
                  $("#map__paragraph_"+i+"__"+pos).attr('id', "map__paragraph_"+i+"__"+posid);
              }
              else if(!document.getElementById("map__paragraph_"+i+"__"+pos) && document.getElementById("map__paragraph_"+i+"__"+posid)){
                  $("#map__paragraph_"+i+"__"+posid).attr('name', "map__paragraph_"+i+"__"+pos);
                  $("#map__paragraph_"+i+"__"+posid).attr('id', "map__paragraph_"+i+"__"+pos);
          ...
          setTimeout(function() 
          {              
             document.getElementById('preview__'+posid).contentWindow.document.body.innerHTML = ifr; 
              document.getElementById('preview__'+pos).contentWindow.document.body.innerHTML = ifr1;
              $('#preview__'+pos).attr('onload', "resizeIframe(this, 'LoadZoneLeft_"+pos+"')");
              $('#preview__'+posid).attr('onload', "resizeIframe(this, 'LoadZoneLeft_"+posid+"')");
              for(i=0;i<100;i++){
                  if((loadzl1[i] == null || loadzl1[i] == 'undefined' || loadzl1[i] == 'null') && (loadzl[i] == null || loadzl[i] == 'undefined' || loadzl[i] == 'null')){
                      break;
                  }
                    ...
                    if(loadzl1[i] != null && loadzl1[i] != 'undefined' && loadzl1[i] != 'null' && (loadzl[i] == null || loadzl[i] == 'undefined' || loadzl[i] == 'null')){
                       $("#map__paragraph_"+i+"__"+pos).html(loadzl1[i]);
                       var cke_map = "map__paragraph_"+i+"__"+pos;
                       var cke_map_des = "map__paragraph_"+i+"__"+posid;
											
                       CKEDITOR.instances[cke_map_des].destroy();
                       CKEDITOR.replace(cke_map,{toolbar: 'Basic',
                                                    height: 100,
                       ...
                       window.CKEDITOR.instances[cke_map].on('change', function() { 
                            onParagraphChange(this);
                        });
                        window.CKEDITOR.instances[cke_map].on('instanceReady', function() { 
                            setIframeHeight('preview__'+pos, 'LoadZoneLeft_'+pos);
                        });
                    }
                    else if(loadzl[i] != null && loadzl[i] != 'undefined' && loadzl[i] != 'null' && (loadzl1[i] == null || loadzl1[i] == 'undefined' || loadzl1[i] == 'null' )){
                       $("#map__paragraph_"+i+"__"+posid).html(loadzl[i]);
                       var cke_map_1 = "map__paragraph_"+i+"__"+posid;
                        var cke_map_2 = "map__paragraph_"+i+"__"+pos;
											  
                       CKEDITOR.instances[cke_map_2].destroy();
                       CKEDITOR.replace(cke_map_1 ,{toolbar: 'Basic',
                                                    height: 100,
                                                    removePlugins: 'elementspath',
                                                    resize_enabled: false,
                                                    fillEmptyBlocks: false,
                       ...
                       window.CKEDITOR.instances[cke_map_1].on('change', function() { 
                            onParagraphChange(this);
                        });
                        window.CKEDITOR.instances[cke_map_1].on('instanceReady', function() { 
                            setIframeHeight('preview__'+posid, 'LoadZoneLeft_'+posid);
                        });
                    }    
                    else if((loadzl[i] != null && loadzl[i] != 'undefined' && loadzl[i] != 'null') && (loadzl1[i] != null && loadzl1[i] != 'undefined' && loadzl1[i] != 'null' )){
                        $("#map__paragraph_"+i+"__"+pos).html(loadzl1[i]);
                ...
                $("input[name$='magazine_content["+pos+"][content_id]']").attr('name', "magazine_content["+pos+"][content_id]_temp");
            $("input[name$='magazine_content["+posid+"][content_id]']").attr('name', "magazine_content["+pos+"][content_id]");
            $("input[name$='magazine_content["+pos+"][content_id]_temp']").attr('name', "magazine_content["+posid+"][content_id]");

            $("input[name$='magazine_content["+pos+"][position]']").attr('name', "magazine_content["+pos+"][position]_temp");
            $("input[name$='magazine_content["+posid+"][position]']").attr('name', "magazine_content["+pos+"][position]");
            $("input[name$='magazine_content["+pos+"][position]_temp']").attr('name', "magazine_content["+posid+"][position]");

            $("input[name$='magazine_content["+pos+"][template_id]']").attr('id', "template_id__");
            $("input[name$='magazine_content["+posid+"][template_id]']").attr('id', "template_id__"+pos);
            $("input[name$='magazine_content["+pos+"][template_id]']").attr('id', "template_id__"+posid);

            $("input[name$='magazine_content["+pos+"][template_id]']").attr('name', "magazine_content["+pos+"][template_id]_temp");
            $("input[name$='magazine_content["+posid+"][template_id]']").attr('name', "magazine_content["+pos+"][template_id]");
            $("input[name$='magazine_content["+pos+"][template_id]_temp']").attr('name', "magazine_content["+posid+"][template_id]");
            // set lại name,id ảnh 
            for(h=0;h<20;h++){
                if ($('input[name="old_map__image_'+h+'__'+pos+'"]').length >0 && $('input[name="old_map__image_'+h+'__'+posid+'"]').length >0){
                    $("input[name$='old_map__image_"+h+"__"+pos+"']").attr('name', "old_map__image_"+h+"__temp");
                    $("input[name$='old_map__image_"+h+"__"+posid+"']").attr('name', "old_map__image_"+h+"__"+pos);
                    $("input[name$='old_map__image_"+h+"__temp']").attr('name', "old_map__image_"+h+"__"+posid);
                }
                if(document.getElementById("old_map__image_"+h+"__"+pos) && document.getElementById("old_map__image_"+h+"__"+posid)){
                    $("input[name$='old_map__image_"+h+"__"+pos+"']").attr('id', "old_map__image_"+h+"__");
                    $("input[name$='old_map__image_"+h+"__"+posid+"']").attr('id', "old_map__image_"+h+"__"+pos);
                    $("input[name$='old_map__image_"+h+"__"+pos+"']").attr('id', "old_map__image_"+h+"__"+posid);
                }
            }
            // Gắn cờ sau khi đã hoàn thành chuyển đổi
            if (v_allow_action_down_up == 2){
                v_allow_action_down_up = 1;
            }
          }, 500);
        } else {
            // Gắn cờ sau khi đã hoàn thành chuyển đổi
   ...
   function no2down(e,pos) {
            var sub = document.getElementById(e);
            move(sub,pos,0);
    }

    function no2up(e,pos) {
            var sub = document.getElementById(e);
            move(sub,pos,1);
    }
//    END tuannt bo sung chuc nang len xuong khoi noi dung magazine
  //Begin AnhTT 22/4/2020 toi_uu_magazine

function changePositionStt(obj){
    arrTempOrderObj[arrTempOrderObj.length] = obj;
    arrTempOrderValue[arrTempOrderValue.length] = parseInt(obj.value);
    objAll = document.getElementsByClassName('magazine-content-number');
    if(objAll.length > 0){
        var j=1;
        for(i=0;i<objAll.length;i++){
            if(arrTempOrderObj.indexOf(objAll[i]) >= 0){
                    continue;
            } 
            while (arrTempOrderValue.indexOf(j) >= 0) {
              j = j+1;
            }
            objAll[i].value = j;
            j++;
 ...
 * */
function load_div_loai_video_24h(value,txtdiv){
    // nếu là loại quảng cáo nivea, thì hiển thi div chọn loại giải đấu
    if(value == 1){
        document.getElementById('div_loai_video_nivea'+txtdiv).style.display = 'block';
    }else{
        document.getElementById('div_loai_video_nivea'+txtdiv).style.display = 'none';
    }
}