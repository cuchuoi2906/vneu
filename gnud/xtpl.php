...
/*
	...
	and http://sourceforge.net/projects/xtpl/
...
function text($bname='main') {
	if (!isset($bname))
		$bname=$this->mainblock;
	return $this->parsed_blocks[$bname];
}

function content( $bname='main') {
	$this->parse( $bname);
	return $this->text( $bname);
}

function display( $bname='main') {
	$this->parse( $bname);
...
/***[ assign_file ]***********************************/
/*
	add by dungpt@ashui.com
*/

function assign_file( $name, $file){
	$this->filecontents = str_replace( '{'.$name.'}', $this->r_getfile( $file), $this->filecontents);
	$this->blocks = array();
	$this->blocks=$this->maketree($this->filecontents,$mainblock);	/* preprocess some stuff */
	$this->scan_globals();
}
