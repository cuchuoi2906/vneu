...
{   
    ...
    curl_close($ch);
	if (GNUD_DEBUG_MODE) {
		fw24h_write_log(date("Y-m-d H:i:s ").$url."\n\n", WEB_ROOT."logs/request_services_tier.log");
	}	
    return $result;
} 
...
// Ham thay the ky tu bang cache key_html
function filter_content($contents)
{		
	$v_content = $contents['array']['data'];
	if ($v_content!='') {
		$contents['gnud_request_type'] = 'file';
	}
	return $contents;	
}